...
import { 
  ...
  Moon, Sun, CheckCircle2, Zap, Boxes, Compass, Layers, Printer
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#06040A] text-slate-900 dark:text-slate-100 transition-colors duration-300 font-sans selection:bg-purple-500/30">
      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white/80 dark:bg-[#0a0614]/80 backdrop-blur-lg border-b border-slate-200 dark:border-white/10' : 'bg-transparent'}`}>
        <div className="container mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-9 h-9 bg-purple-600 rounded-lg flex items-center justify-center text-white font-extrabold text-lg shadow-md shadow-purple-500/20">T</div>
            <span className="text-xl font-extrabold tracking-tight bg-gradient-to-r from-purple-600 to-indigo-500 bg-clip-text text-transparent">Textile ERP</span>
          </div>
      ...
      <section className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden">
        {/* Background glow */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-purple-500/10 dark:bg-purple-600/10 blur-[120px] rounded-full pointer-events-none"></div>
        ...
        <div className="container mx-auto px-6 relative z-10 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white dark:bg-white/5 border border-purple-200 dark:border-purple-500/20 shadow-sm mb-8 animate-fade-in-up">
            ...
            <span className="text-sm font-medium text-purple-700 dark:text-purple-300">Advanced Textile & Garment Manufacturing Suite</span>
          </div>
          
          <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight mb-8 leading-tight max-w-4xl mx-auto">
            Powering the Future of <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-indigo-500">Textile Production</span>
          </h1>
          
          <p className="text-lg md:text-xl text-slate-600 dark:text-slate-400 max-w-2xl mx-auto mb-12 leading-relaxed">
            Manage fabric procurement, yarn calculations, piece-rate wages, and shop floor processing with real-time barcode tracking. Scaled for spinning, weaving, printing, and apparel factories.
          </p>
      ...
      {/* Textile Production Flow Section */}
      <section className="py-24 bg-white dark:bg-[#0B0815] relative border-y border-slate-100 dark:border-white/5">
        <div className="container mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-bold mb-4">Textile & Apparel Pipeline</h2>
            <p className="text-slate-500 dark:text-slate-400 text-lg max-w-2xl mx-auto">Digitize every workflow from raw yarns and rolls to finished boxes.</p>
          </div>
          ...
          <div className="grid md:grid-cols-4 gap-8 relative">
            ...
            <FlowStep icon={<Layers />} step="1. Fabric & Design" desc="Configure fabric specs (GSM, blend), define design files, and calculate fabric consumption." />
            <FlowStep icon={<Scissors />} step="2. Cut & Create Lots" desc="Manage layering, allocate fabric sizes (S, M, L, XL, XXL) and quantities, and register production lots." />
            <FlowStep icon={<QrCode />} step="3. Scan Floor Stages" desc="Track lot movements via barcode scans across Printing, Embroidery, Checking, Stitching, and Ironing." />
            <FlowStep icon={<Boxes />} step="4. Pack & Ship" desc="Print box labels, manage warehouse inventories, verify quality compliance, and dispatch to customers." />
          </div>
        </div>
      </section>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="space-y-8">
              <h2 className="text-3xl md:text-5xl font-bold leading-tight">
                Specially tailored for <br/><span className="text-purple-600 dark:text-purple-400">textile manufacturers.</span>
              </h2>
              <p className="text-lg text-slate-600 dark:text-slate-400">
                General ERP solutions fall short on apparel complexities. Textile ERP is custom-built to calculate piece-rates, track garment stages, and isolate tenant company databases securely.
              </p>
              
              <ul className="space-y-6">
                <FeatureListItem title="Fabric & Recipe Master" desc="Save fabric specifications like Cotton, Linen, and Polyester composition, weave types, and GSM." />
                <FeatureListItem title="Piece-rate wages & Payroll" desc="Register check-ins instantly with employee QR code badges. Automate wages by production yield." />
                <FeatureListItem title="SaaS Tenant Security" desc="Each manufacturer operates in a protected environment. Secure logins prevent cross-company data leakage." />
              </ul>
            </div>
            ...
            <div className="relative">
              ...
              <div className="relative bg-white/60 dark:bg-white/5 backdrop-blur-xl border border-white/40 dark:border-white/10 p-8 rounded-3xl shadow-2xl">
                <div className="flex items-center gap-4 mb-8 pb-8 border-b border-slate-200 dark:border-white/10">
                  ...
                  <div>
                    <h3 className="text-xl font-bold">Textile Lot: LOT-2607-01</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400">Fabric: 100% Organic Cotton (220 GSM)</p>
                  </div>
                </div>
                ...
                <div className="mt-8 flex justify-end">
                  <button className="px-6 py-2.5 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-sm font-semibold transition-colors">
                    Active Stage: Stitching
                  </button>
                </div>
              </div>
            </div>
      ...
      <section className="py-24 bg-slate-100 dark:bg-[#0B0815] border-y border-slate-200 dark:border-white/5">
        <div className="container mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-bold mb-4">Flexible Scaling Plans</h2>
            <p className="text-slate-500 dark:text-slate-400 text-lg">Predictable pricing structures designed to grow with your machines and operators.</p>
          </div>

          <div className="grid md:grid-cols-4 gap-6">
            {loadingPricing ? (
              <div className="col-span-4 text-center py-12 text-slate-500">Loading plans...</div>
            ) : (
      ...
      {/* Footer */}
      <footer className="py-12 bg-white dark:bg-[#06040A] border-t border-slate-200 dark:border-white/10">
        <div className="container mx-auto px-6 text-center">
          <div className="flex items-center justify-center gap-2 mb-6">
            <div className="w-7 h-7 bg-purple-600 rounded flex items-center justify-center text-white font-bold text-sm">T</div>
            <span className="font-extrabold text-slate-800 dark:text-white">Textile ERP Suite</span>
          </div>
          <p className="text-slate-500 dark:text-slate-500 text-sm">
            © {new Date().getFullYear()} Textile ERP Platforms. All rights reserved.
          </p>
        </div>
      </footer>
    </div>
