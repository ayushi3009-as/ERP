...
interface InvoiceRow extends PurchaseInvoice {
  vendor_name?: string;
  po_number?: string;
}
...
export default function PurchaseInvoicesPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['purchase-invoices', { search, page, statusFilter }],
    queryFn: async () => {
      const { data } = await api.get<PaginatedResponse<InvoiceRow>>('/v1/purchase-invoices', {
        params: {
          search,
          page,
          per_page: 20,
  ...
  const { data: grnsData } = useQuery({
    queryKey: ['grns', 'for-invoice'],
    queryFn: async () => {
      const { data } = await api.get<PaginatedResponse<GRN>>('/v1/grns', {
        params: { per_page: 200 },
      });
      return data.data;
    },
  ...
  const createMutation = useMutation({
    mutationFn: async (values: InvoiceFormData) => {
      const { data } = await api.post('/v1/purchase-invoices', values);
      return data;
    },
    onSuccess: () => {
  ...
  const paymentMutation = useMutation({
    mutationFn: async ({ invoiceId, amount }: { invoiceId: string; amount: number }) => {
      const { data } = await api.post(`/v1/purchase-invoices/${invoiceId}/payment`, { amount });
      return data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['purchase-invoices'] });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-foreground">Purchase Invoices</h2>
          <p className="text-sm text-muted-foreground">Manage vendor invoices and payment tracking</p>
        </div>
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-5xl">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              <div>
                <label className="text-sm font-medium text-foreground">Linked PO</label>
                <Select
              ...
              <div>
                <label className="text-sm font-medium text-foreground">Linked GRN</label>
                <Select
                  value={watch('grn_id') || ''}
                  onValueChange={(v) => setValue('grn_id', v)}
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    {grns.map((g) => (
                      <SelectItem key={g.id} value={g.id}>{g.grn_number}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                ...
                {viewingInvoice.po_number && (
                  <div>
                    ...
                    <p className="font-medium">{viewingInvoice.po_number}</p>
                  </div>
                )}
              </div>
